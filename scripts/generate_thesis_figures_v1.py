#!/usr/bin/env python3
"""
Generate comprehensive thesis figures from existing results.

This script creates publication-quality visualizations from:
- XGBoost v5 (production model)
- XGBoost v6 (honest evaluation)
- Domain holdout analysis
- Regret analysis
- Model history comparison

All figures saved to results/thesis_figures_v1/ for thesis inclusion.
"""
...
OUTPUT_DIR = RESULTS_DIR / "thesis_figures_v1"
...
def fig_model_history():
    ...
    versions = {
        'v1 (Regressors)': {'test': 44.4, 'real': 57.9, 'domain': None},
        'v2 (Classifier)': {'test': 62.5, 'real': 60.2, 'domain': None},
        'v3 (Log+Pairwise)': {'test': 100.0, 'real': 94.5, 'domain': None},
        'v5 (Production)': {'test': 76.1, 'real': 76.1, 'domain': 88.0},
        'v6 (Honest)': {'test': 71.2, 'real': 71.2, 'domain': None},
    }
    ...
    colors = ['#d62728' if v in ['v1', 'v2'] else '#2ca02c' if v in ['v3'] else '#1f77b4' for v in versions_list]
    ...
    ax1.set_xticklabels(versions_list, rotation=45, ha='right')
    ax1.set_ylabel('Test Accuracy (%)', fontsize=12, fontweight='bold')
    ax1.set_title('Model Evolution: Test Accuracy', fontsize=13, fontweight='bold')
    ax1.set_ylim(0, 105)
    ax1.legend()
    for i, v in enumerate(test_accs):
        ax1.text(i, v + 2, f'{v}%', ha='center', fontweight='bold')
    
    # Plot 2: v5 vs v6 (honest) comparison
    models = ['v5 (Production)', 'v6 (Honest)']
    ...
    ax2.set_title('Production vs Honest Evaluation', fontsize=13, fontweight='bold')
    ax2.set_ylim(0, 100)
...
def fig_feature_importance():
    ...
    ax.set_title('XGBoost v5: Feature Importance (All 16 Features)', fontsize=13, fontweight='bold')
...
def fig_regret_analysis():
    ...
    ax1.set_ylabel('Regret (μs)', fontsize=11, fontweight='bold')
    ax1.set_title('Per-Instance Regret Statistics', fontsize=12, fontweight='bold')
    ...
    algs = ['VBS', 'Model v5', 'Heapsort\n(SBS)', 'Introsort', 'Timsort']
    times = [
        regret['vbs_total_s'],
    ...
    ax2.set_title('Algorithm Total Runtime (1.18M arrays)', fontsize=12, fontweight='bold')
    ...
    ax3.set_title('Regret & Performance Summary', fontsize=12, fontweight='bold')
    ax3.set_ylim(0, 100)
    ...
    ax4.set_title('Arrays with Zero Regret', fontsize=12, fontweight='bold')
...
def fig_vbs_sbs_model():
    ...
    ax.set_title('Algorithm Selection Strategy Comparison (1.18M arrays)', fontsize=13, fontweight='bold')
    ax.set_ylim(0, max(times) * 1.15)
    ax.grid(axis='y', alpha=0.3)
    
    # Add time labels and gap info
...
def fig_domain_holdout():
    ...
    ax.set_title('Domain Holdout: Per-Domain Performance', fontsize=13, fontweight='bold')
    ax.set_xticks(x)
...
def fig_confusion_matrix():
    ...
    if 'predicted' in preds.columns and 'actual' in preds.columns:
        ...
        actual = preds.iloc[:, 1].values  # Likely second column
        predicted = preds.iloc[:, 0].values  # Likely first column
    ...
    ax.set_ylabel('Actual Algorithm', fontsize=12, fontweight='bold')
    ax.set_xlabel('Predicted Algorithm', fontsize=12, fontweight='bold')
    ax.set_title('XGBoost v5: Confusion Matrix (Test Set)', fontsize=13, fontweight='bold')
...
def fig_predictions_distribution():
    ...
    classes = ['introsort', 'heapsort', 'timsort']
    counts = [np.sum(predicted == c) for c in classes]
    ...
    ax1.set_title('v5 Predictions: Algorithm Distribution (Test Set)', fontsize=12, fontweight='bold')
    ax1.grid(axis='y', alpha=0.3)
    ...
    ax2.pie(counts, labels=[f'{c}\n{p:.1f}%' for c, p in zip(classes, percentages)],
           ...
           wedgeprops=dict(edgecolor='black', linewidth=1.5))
    ax2.set_title('Model Selector Bias', fontsize=12, fontweight='bold')
...
def fig_v5_vs_v6():
    ...
    v5_vals = [
        ...
        v5['results']['test'].get('zero_regret_pct', 72) * 100,
    ]
    v6_vals = [
        v6['results']['test']['accuracy'] * 100,
        v6['results']['test']['balanced_accuracy'] * 100,
        v6['results']['test'].get('zero_regret_pct', 45) * 100,
    ]
    ...
    bars2 = ax.bar(x + width/2, v6_vals, width, label='v6 (Honest)', color='#ff7f0e', alpha=0.8, edgecolor='black')
    
    ax.set_ylabel('Percentage (%)', fontsize=12, fontweight='bold')
    ax.set_title('Model Comparison: v5 vs v6', fontsize=13, fontweight='bold')
    ax.set_xticks(x)
...
def main():
    ...
    print("\n" + "="*70)
    print("GENERATING THESIS FIGURES v1")
    print("="*70)
