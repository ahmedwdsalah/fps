...
========================================================================
Identical to train_xgboost_v5.py, but WITH figure generation during training.
...
DIFFERENCES FROM train_xgboost_v5.py:
  ✅ Adds matplotlib imports for figure generation
  ✅ Tracks metrics per epoch
  ✅ Generates ROC curves
  ✅ Generates PR curves
  ✅ Generates learning curves
  ✅ Outputs to results/xgboost_v5_with_figures/ (doesn't overwrite v5)

Inputs:  data/training_dataset.csv  (from Step 2)
Outputs: models/xgboost_v5_with_figures/        (model JSON)
         results/xgboost_v5_with_figures/       (metrics + FIGURES)
...
def generate_pr_curves(y_true, y_proba, output_path):
    ...
    print(f"[FIG] Generating Precision-Recall curves...")
...
def main():
    print("=" * 80)
    print("XGBOOST v5 WITH FIGURES — Training Real-World Balanced Classifier")
    print("=" * 80)
    ...
    margin_filtered = df[keep].reset_index(drop=True)
    ...
    # Split: 70/15/15 (stratified) - split as 70/30, then 50/50 the 30%
    ...
    for split_name, X, y_true, y_true_enc in [
        ...
        results[split_name] = {
            ...
            'confusion_matrix': confusion_matrix(y_true, y_pred, labels=ALGORITHMS).tolist(),
            'classification_report': classification_report(y_true, y_pred, labels=ALGORITHMS, output_dict=True, zero_division=0),
        }
    ...
    y_test_np = np.array([le.transform([y])[0] for y in predictions['test']['y_true']])
    y_pred_test = np.array([le.transform([y])[0] for y in predictions['test']['y_pred']])
