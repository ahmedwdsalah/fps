...
- Main deck target: 18-22 slides, excluding backup.
...
- Model-track strategy: v5 is the main story. F1 channel/flag models get a brief mention in main deck and detailed backup slides.
- Rigor strategy: include one main slide on negative results / lessons (`v7`, `v8`, source-aware checks), with deeper details in backup.
- Backup deck: deep technical backup covering feature formulas, SBS/VBS/regret definitions, v6/source-aware checks, v7/v8, F1 routing, extra confusion matrices, and likely committee Q&A.
- Slide density: sparse main slides with spoken detail in concise speaker notes.
- Backup size: 12-16 slides focused on committee-risk defense.
...
- Backup placement: place backup slides after the thank-you/Q&A slide in the same deck.
...
- Research journey: one compact main slide only. Keep detailed model history in backup.
...
- F1 strategy: brief main-deck caveat only. Detailed F1 channel/flag routing belongs in backup.
...
- Backup priority: committee-defense risks first, especially leakage, v6, F1 routing, metric definitions, feature cost, and why `76.1%` accuracy is still valuable.
...
- Backup slides can be more technical, but still should answer committee questions directly.
...
- Prefer one central idea per slide. If a slide needs many bullets to be understood, split it, move details to speaker notes, or put the detail in backup.
...
- Replace internal wording with natural defense wording:
  - “backup inventory” -> “extra slides for questions”
...
Target length: 22 main slides, excluding backup. If time pressure forces trimming, merge slides 3-4 or slides 20-21.
...
12. The main model is the general v5 selector
   ...
   - Speaker-note intent: say v5 is the submitted general model; F1 routing is a separate specialized experiment for questions or backup.
...
## Backup Slide Inventory

Prioritize backup slides that defend likely committee risks, not slides that simply add more content.

- Source leakage / v6 source-aware check:
  - Explain 37,976 arrays, 10,792 sources, 9 domains, GroupShuffleSplit by `source_id`.
  - State that stricter evaluation lowers headline metrics but preserves the same interpretation.
- v7 regret-aware failure:
  - Standard argmax performed better than regret-aware decision mode.
  - Use as evidence that the final method was chosen empirically, not because it sounded attractive.
- v8 binary cascade:
  - Stage 1 timsort-vs-rest was strong, Stage 2 introsort-vs-heapsort was weak.
  - Key point: current feature space makes the problem effectively timsort detection plus a hard low-level boundary.
- F1 channel classifier and routed models:
  - Separate clearly from v5.
  - Explain F1-specific channel/flag assumptions and the later 5-algorithm `actual_5` label set.
  - Include strict vs non-strict routing caveat.
- Feature formulas and extraction cost:
  - Show formulas only if asked.
  - Be precise that features are cheap structural features, not all strictly one-pass.
- SBS/VBS/regret formulas:
  - Keep exact definitions available for Q&A.
  - Use these to defend why `76.1%` accuracy can still produce `93.1%` gap closed.
- Extra confusion matrices and per-domain metrics:
  - Use if asked about class imbalance, timsort dominance, weather/F1 limits, or domain generalization.
- Model and portfolio choices:
  - Why XGBoost, why classification instead of regression, why three algorithms, why counting/radix/custom Python-loop sorts were excluded.
...
- v5-v6 backup slide: `results/thesis_figures/10_v5_vs_v6_comparison.png`
- F1 backup slides: `results/f1_dynamic_figures/01_strict_metrics.png`, `02_channel_balanced_accuracy.png`, `03_pipeline.png`, `04_strict_split_sizes.png`
...
- Use F1 dynamic/routing figures only in backup or if asked during Q&A.
